 ...
 **********************************************************************/
module ext.load.libload;
import core.stdc.stdlib, std.stdio, std.conv, std.string;
import ext.load.loader;
...
protected HXModule load_library(string win_name, string linux_name = "", string osx_name = "", bool extension=true){
  ...
  if((shared_library = ExeModule_Load(full_name)) is null){
    writefln("Shared library %s not found on PATH", full_name.toUpper());
    exit(0);
  }
  debug writefln("Shared library: %s loaded", full_name);
  ...
  bool opCall(HXModule lib, string name) {
    try{
      *fptr = getFunctionThroughVoid(lib, name);
      debug writefln("Loaded shared function: %s", name);
      return true;
    }catch(Exception e){
      writefln("Unable to load shared function: %s", name);
      return false;
    }
  }
